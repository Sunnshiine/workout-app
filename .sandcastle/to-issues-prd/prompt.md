...
2. Read `CONTEXT.md` and skim `docs/adr/` for any decisions that bear on the area the PRD touches. For product or UI PRDs, also read `PRODUCT.md` and `DESIGN.md`. Sub-issue titles and bodies must use the project's vocabulary (Block, Week, Session, Exercise, Set, Move On, Load Suggestion, …).
...
- Each slice delivers a narrow but COMPLETE path through every layer it needs.
...
- Sub-issues run in **list order** under the PRD. Order them so dependencies are satisfied: if slice B builds on slice A's model changes, A must come first.
- Each slice must stand on its own in a single agent session. A reasonable session can build a couple of files, write tests, and run `swift test`. Don't draft slices that are unrealistic for one session.
