...
2. Make the code changes you decided on. Run `swift test` before committing. If your changes touch the app target (anything under `App/Views/`, `WorkoutTrackerApp.swift`, or other code the SPM library target doesn't compile), also compile-check it — the workflow pre-created `Secrets.xcconfig` from the template: `xcodebuild build -project WorkoutTracker.xcodeproj -scheme WorkoutTracker -destination 'generic/platform=iOS Simulator' -skipPackagePluginValidation CODE_SIGNING_ALLOWED=NO`. If a change you make alters anything the user can see, complete both mechanisms of `.sandcastle/VISUAL_LOOP.md` — **record-and-look** and **eyes-on-app** — before committing. Use conventional-commit messages (`feat:`, `fix:`, `refactor:`, etc.).
