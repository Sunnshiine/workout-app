...
2. Make the code changes you decided on. Run `swift test` before committing. If your changes touch the app target (anything under `WorkoutTracker/Views/`, `WorkoutTrackerApp.swift`, or other code the SPM library target doesn't compile), also compile-check it — the workflow pre-created `Secrets.xcconfig` from the template: `xcodebuild build -project WorkoutTracker.xcodeproj -scheme WorkoutTracker -destination 'generic/platform=iOS Simulator' -skipPackagePluginValidation CODE_SIGNING_ALLOWED=NO`. If a change you make alters anything the user can see, read `.sandcastle/VISUAL_LOOP.md` and follow it before committing (record the affected Visual Baselines and look at them; drive the changed screens in the fixture app via XcodeBuildMCP and screenshot both appearances). Use conventional-commit messages (`feat:`, `fix:`, `refactor:`, etc.).
