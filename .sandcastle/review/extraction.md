...
<output>
{
  "summary": "Fixed a null-dereference in `getUser` and added a guard clause. The original code assumed `ctx.user` was always present, but it can be `undefined` after token expiry. Also flagging an unrelated naming inconsistency in `OpenExercises.swift`.",
  "inlineComments": [
    {
    ...
    {
      "path": "Sources/WorkoutTracker/Progress/OpenExercises.swift",
      "line": 14,
...
| `inlineComments[].path` | string  | **yes**  | Relative file path, e.g. `"Sources/WorkoutTracker/Models/Session.swift"`.                                                                                                                                                                                                       |
