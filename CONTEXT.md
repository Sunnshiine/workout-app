...
**Set State**: The status of a single Set — Pending (not yet logged or skipped), Logged (any non-empty value in the Notes cell), or Skipped (athlete explicitly marked it skipped; "skip" written to the sheet so the coach can see it). A Logged Set may or may not have structured data (weight, reps, RPE) — an unstructured log still counts as Logged for progress tracking. Avoid: done, missed, incomplete.
