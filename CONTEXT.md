...
**Rest**: A countdown the app starts automatically the moment the athlete records an intentional Set Log, pacing recovery before the next Set. Purely local and never written to the Sheet. At most one Rest exists at a time; recording another Set Log restarts it. A Rest is tied to its origin Set Log and the Session it was logged in: it ends when it expires, when a new Set Log restarts it, when the athlete dismisses it, when the origin Set Log is deleted, or when the athlete chooses Move On out of that Session. A Rest does **not** start when the athlete edits an existing Set Log, skips a Set, or when the Set Log being recorded leaves no Pending Set anywhere in the Current Week. The Rest never blocks: the athlete may keep logging, move on, or do nothing while it runs. Avoid: rest timer screen, break, interval, cooldown.

**Rest Duration**: The length a Rest counts down from, chosen by the athlete in Settings. Two values exist: the **Standard Rest Duration**, used for an ordinary Set Log, and the **Superset Rest Duration**, a shorter value used when the just-logged Set's Exercise belongs to a Superset (where the athlete intends to rest less). The Superset Rest Duration applies to any Set logged in an Exercise that is part of a Superset pairing. Avoid: rest length, timer setting.
