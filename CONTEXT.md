...
**Exercise History**: A view of an Exercise's prior entries across Sessions — an **entry** is that Exercise as logged in one past Session. Shows the last ~5 entries, collected at Movement level so spelling variants and Cadence prefixes never silo history; entries display in strict recency order and annotate themselves with their own entered name only when it differs beyond case from the viewed Exercise's. Opened mid-Session by tapping the Last Performed line. Distinct from Last Performed, which is only the single most recent entry. Avoid: Last Performed, previous result, performance history.
