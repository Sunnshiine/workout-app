...
**Coach Note**: Instruction-shaped text the coach places in the Notes column (J) on the Exercise header row — e.g. "Start w/ 10 sec hold, proceed to rep range" or "Superset w/...". Read-only to the athlete. Never overwritten by the app.

**Legacy Anchor Result**: Result-shaped athlete text found in the Notes column (J) on an Exercise header row from older logging habits or app behavior — e.g. "70@10, 55", "55x8, 60x7@9.5", or "70, 80, 90x6". Counts all prescribed Sets for that Exercise as complete and can serve as Last Performed evidence when structured continuation-row Set Logs are absent, but is not a Coach Note and is not a structured Set Log unless intentionally migrated into Set continuation rows. Avoid: coach note, current log.

**Set State**: The status of a single Set — Pending (not yet logged or skipped), Logged (any non-empty value in the Notes cell), or Skipped (athlete explicitly marked that Set skipped; "skip" written to the sheet so the coach can see it). Skipping is Set-level; an Exercise is complete when all prescribed Sets are Logged or Skipped, or when a Legacy Anchor Result completes the Exercise. Avoid: done, missed, incomplete.
