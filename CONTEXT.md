...
**Unstructured Set Log**: Non-empty athlete text on a Set continuation row that marks that Set Logged but does not parse as a structured Set Log. It is Set-level completion evidence, distinct from a Legacy Log, and may be overwritten by a structured Set Log when the athlete corrects it. Avoid: completed from sheet, legacy log, already logged.
...
**Set State**: The status of a single Set — Pending (not yet logged or skipped), Logged (a Set Log or Unstructured Set Log on the Set continuation row), or Skipped (athlete explicitly marked that Set skipped; "skip" written to the sheet so the coach can see it). Skipping is Set-level; an Exercise is complete when all prescribed Sets are Logged or Skipped, or when a Legacy Log completes the Exercise. Avoid: done, missed, incomplete.
