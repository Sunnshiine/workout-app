...
**Performance History**: A quick mid-Session reference view of an Exercise's recent prior performances — the last ~5 across Sessions and Blocks — read primarily to pick today's starting weight, secondarily for short-term trajectory. Opened by tapping the Last Performed line on the Exercise stage. Matches at the movement level: fuzzy matching across coach spelling variants, never siloed by Cadence — all Cadence variants of a movement share one history, biased toward over-matching. Only performances with completion evidence appear: fully Skipped occurrences are omitted, partially Skipped performances show their "skip" markers inline, and Legacy Logs display as raw entered text. Deliberately shallow — early occurrences of a movement understate capacity while form settles, and loads from several Blocks back mislead as targets. Distinct from Last Performed, which is only the single most recent Logged result. Avoid: Last Performed, previous result, full history, archive.
