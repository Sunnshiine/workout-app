...
/// The single interface onto the Last Performed owner (PRD #330): ingest evidence and count
/// coverage. Callers — the Exercise History fill, in particular — hold this one seam rather than a
/// bare `ModelContext`, so ingest can refresh the display snapshot as part of the same operation
/// and the write→refresh pairing cannot drift.
@MainActor
