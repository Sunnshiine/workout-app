...
/// Set is a **carved chip** — the app's only below-flat elevation — flowing off a `Wn Dn` gutter
/// under quiet muted Block headers, newest first.
///
/// Skips, Legacy Log rawness, and fallback-spelling annotations never sit in the ledger: they hide
/// behind a `*` on the W/D label that expands into a small carved well. A Volume control (off by
/// default, pressed-in when active) summons the one chart — total volume per Session, hollow `≈`
...
struct ExerciseHistorySheet: View {
    ...
    /// The Volume control — a chip-vocabulary pill, flat at rest, pressed below flat when active
    /// (it borrows the carved-chip recipe; DESIGN.md §The History Is Carved Rule). No action green.
    private var volumeControl: some View {
        Button {
            ...
            Text("Volume")
                ...
                .background {
                    if showVolume {
                        Capsule().fill(palette.chipCarvedFill)
                    } else {
                        Capsule()
                            .fill(palette.footFill)
    ...
    private func blockSection(_ block: ExerciseHistorySheetPresentation.Block) -> some View {
        VStack(alignment: .leading, spacing: 12) {
            // A quiet muted Block header — no rule, no uppercase editorial scaffolding beyond the
            // header's own uppercased tab name.
            Text(block.header)
                .font(Theme.font(.queuePill))
                .foregroundStyle(palette.textSecondary)
                .tracking(0.6)
...
private struct ExerciseHistoryRow: View {
    ...
    private func chipView(_ chip: ExerciseHistorySheetPresentation.Chip) -> some View {
        ...
        .padding(.vertical, 4)
        // Carved: the app's only below-flat elevation. A low-opacity ink fill inset into the paper,
        // with a hairline top highlight to read as pressed below the surface.
        .background {
            Capsule()
                .fill(palette.chipCarvedFill)
                .overlay(
                    Capsule()
    ...
    private var well: some View {
        ...
        .padding(.horizontal, 12)
        .padding(.vertical, 8)
        .background(palette.chipCarvedFill, in: .rect(cornerRadius: Theme.Radius.cell))
        .accessibilityElement(children: .combine)
...
/// a quiet ink line, solid dots for exact totals, hollow `≈` dots where Legacy Logs make a total
/// best-effort, and a dotted seam wherever the line crosses a Block boundary. No axes, no labels —
...
private struct VolumeChart: View {
    ...
    private func blockSeams(positions: [CGPoint], height: CGFloat) -> some View {
        ForEach(seamIndices(), id: \.self) { index in
            ...
            .stroke(palette.blockSeam, style: StrokeStyle(lineWidth: 1, dash: [3, 4]))
        }
    }

    private func dots(positions: [CGPoint]) -> some View {
        ForEach(points.indices, id: \.self) { index in
            dot(approximate: points[index].approximate)
                .position(positions[index])
        }
    }

    @ViewBuilder
    private func dot(approximate: Bool) -> some View {
        if approximate {
            // Hollow `≈` dot: the total is best-effort, so the mark is an open ring on the paper.
            Circle()
                .strokeBorder(palette.chartLine, lineWidth: 1.5)
