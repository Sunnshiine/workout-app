...
- **Clear the skip.** Run `verify.sh tap --label "Set 2, skip"`. The card reads `Set 2 of 3` with a `Skipped` text. Run `verify.sh tap --id clear-logged-set-menu`, then `verify.sh tap --label Clear`. The leaf returns to `Set 2, 5 · RPE7` and `session-remaining-count` reads `4 Sets left`.
