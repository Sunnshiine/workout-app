...
- The Reps and RPE rails draw only about three chips at a time and recentre on the selection, so most chips sit outside the drawn track. Neither `tree` nor `find` warns, because both measure against the 402-point screen rather than the track. Hit testing is not clipped either, so `tap --id rpe-7` really does select 7 while nothing the user can see moves. Tap only a chip the rail draws (with `rpe-6` selected that is `rpe-5` through `rpe-6.5`) and step one chip at a time, since each tap slides the next one in. A chip past the screen edge is worse still. There the tap is clamped back onto the screen and lands on the other rail, where `tap --id rpe-7.5` set reps to 11.
