...
- **Inert tile.** Tap a `Not uploaded` tile. Run `verify.sh tap --id session-tile-W1-D3`. The `Block 27` heading stays and `verify.sh tree --all | grep "Bench Press"` prints nothing.
