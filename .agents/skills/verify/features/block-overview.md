...
- **Open.** Tap an uploaded tile. Run `verify.sh tap --id session-tile-W1-D2`. `find stage-exercise-name` reads `Bench Press`, `find session-location-button` reads `Open Block Overview for Week 1, Day 2`, and `make-current-session-button` and `go-back-current-session-button` appear.
- **Make current.** Run `verify.sh tap --id make-current-session-button`. `verify.sh find make-current-session-button` and `verify.sh find go-back-current-session-button` both exit 1. Run `verify.sh tap --id session-location-button`. `session-tile-W1-D2` is valued `Current` and `session-tile-W1-D1` `Incomplete`.
