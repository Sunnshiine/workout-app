...
- **Open.** Tap the queue button. Run `verify.sh tap --id stage-queue-button`. The tree has `This Session`, `stage-queue-row-exercise-0` labeled `Primer Row`, and `stage-queue-row-exercise-1` labeled `Back Squat, Now`. `Primer Row` is finished, so `verify.sh tap --id stage-queue-row-exercise-0` refuses after its 3 s, exits 1 with `disabled` on stderr, and leaves the sheet open.
...
- **Up next.** Relaunch on the smaller fixture. Run `verify.sh stop`, then `verify.sh launch session`. `verify.sh find stage-queue-button` reads `0 of 2` and `find stage-up-next` reads `Up next ·, 2-3:1:0 BB RDL`. Run `verify.sh tap --id stage-up-next`. `find stage-exercise-name` reads `BB RDL` and the button reads `Up next ·, Back Squat`. Tap it again to return.
...
- **Complete pairing.** Run `verify.sh tap --id stage-queue-pair-exercise-0`, then `verify.sh shot queue-pairing`, then `verify.sh tap --id stage-queue-row-exercise-1`, wait a second, and run `verify.sh shot superset-row`. One row, `stage-queue-row-superset-0` labeled `Back Squat + BB RDL, Now`, replaces both, `verify.sh find stage-queue-row-exercise-1` exits 1, and the queue button reads `0 of 1`. The changed lines also drop `stage-up-next` from the stage, because nothing is queued behind a single item. Run `verify.sh tap --id stage-queue-row-superset-0`. The sheet closes and the stage shows `superset-partner-name` labeled `& BB RDL` under `Back Squat`, which the `superset-row` shot already caught behind the still-open sheet.
- **Proof.** On the sheet, `queue-top` and `queue-tail` are the half-height sheet either side of one `swipe up`. The top cell clips `Cable Crunch` at the bottom edge and the tail cell clips `Back Squat` at the top, so neither alone holds all eight rows, and `Primer Row` is the one grey row, which is what `disabled` looks like. `queue-pairing` and `superset-row` are the pair that proves the merge. Two rows carrying three and two set pips become one row carrying five under `Back Squat + BB RDL`, the title flips from `Pick a partner` back to `This Session`, and a green `Move On` fills the space the second row left. The green link glyph in `queue-pairing` marks only the row the pairing started from.
