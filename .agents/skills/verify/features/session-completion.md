...
- **Continue to the grid.** Launch `partial-block`. Run `verify.sh tap --id session-tile-W4-D1`, `verify.sh tap --id make-current-session-button`, `verify.sh tap --id stage-queue-button`, `verify.sh tap --id queue-move-on-button`. `find move-on-celebration` is labeled `Week 4, Day 1` and its value ends `1 Sets, 1 Exercises, 1 Left`. Run `verify.sh tap --id move-on-celebration-continue`. The `Block 27` heading returns, `verify.sh find move-on-celebration` exits 1, and `session-tile-W4-D2` reads `Not uploaded`. Run `verify.sh shot 03-grid`.
- **Proof.** On the sheet, the `01-complete` cell shows `Session complete` over `1 set done across 1 exercise`, the `Open Exercises` card holding `Back Squat` with `1 pending set` and a `W1 D1 ›` chevron, and the green `Move On` button beside the `1 of 1` queue pill. The `02-celebration` cell is the one the recipe exists to capture. It shows `Day 2, done.` set large over the vine and bird illustration, the coach line centred under it, and a card reading `1` `1` `0` above `Sets`, `Exercises`, `Left`, with `Continue` at the foot. Read the coach line, never assert it. The `03-grid` cell shows the Block 27 grid with Week 4 raised on a lighter card, its Day 1 tile outlined as `Current` and the other three drawn as dashed outlines, which is what `Not uploaded` looks like.
...
- The celebration is an overlay and the stage stays in the tree underneath it. `Session complete`, `move-on-button`, and `stage-queue-button` all still answer while it is up, so none of them proves it was dismissed; only `find move-on-celebration` does. `move-on-celebration-continue` at `@22,734` also covers `move-on-button` at `@16,726`, so dismiss by `--id` and never by coordinates.
