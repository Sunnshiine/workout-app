...
- **Developer Tools.** Run `verify.sh tap --id settings-developer-tools-row`. A `Developer Tools` heading appears with `Current Session Debug Info`, `current-session-debug-resolved-value` reading `Week 1, Day 1`, `Pending Sheet Writes`, and `developer-tools-sync-button`. Tap `--label Settings` in the navigation bar to return.
...
- `developer-tools-sync-button` sits below the fold at `@32,977` on a 402x874 screen. `tap --id` taps the frame centre whether or not it is on screen, and a tap past the bottom edge reports success while hitting nothing. Swipe up twice first and confirm `find` reports a `y` under 874.
