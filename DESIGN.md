...
The accepted direction is F3 Static Shell: a quiet full-screen shell that uses the active appearance palette in both Dark and Sage Light. The TFN mark sits at top-left, `Week N · Day X` sits at top-right, `Move On` introduces the message, and the rotating approved quote is the main copy. A concise saved/open Sets line follows the quote, the `Sets / Exercises / Left` stats row is the only block-like surface, and `Tap anywhere to continue` closes the screen.

The quote carries the product's motivating personality, but it must not become a fragile oversized hero. Give the quote a stable visual region that can show longer copy with slight scaling before relying on the screen's scroll fallback.
...
Do not add elapsed-time UI, timing rows, Exercise cards, route checkpoints, confetti, XP, badge stacks, mascots, or decorative bloom/ripple motion to the default hierarchy.

**The Celebration Has One Block Rule.** Move On Celebration uses the `Sets / Exercises / Left` stats row as its only block-like surface. Do not reintroduce a checkmark stamp, badge stack, mascot, confetti, Exercise cards, route preview, or multiple competing celebration graphics.
...
**The No Fixed Day Count Copy Rule.** Do not use phrases like "only three days left" or "halfway there" unless they are derived from the actual number of Sessions in the Week. Future Weeks may have two to seven Sessions.
