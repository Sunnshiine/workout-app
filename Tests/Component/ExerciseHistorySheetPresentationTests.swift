...
@Test func fullyUnparseableEntrySurfacesItsRawLineInTheWellAndIsNeverChipless() {
    // The re-drive decision (addendum §7.5): a fully-unparseable Legacy Log yields no chips, but the
    ...
    let row = presentation.blocks[0].rows[0]
    #expect(row.chips.isEmpty)
    // The row is never chipless-and-empty: the `*` well represents it, holding the raw Sheet line.
    #expect(row.hasWell)
    #expect(row.annotations == [.asEntered(raw)])
    // An uncomputable total is omitted rather than plotted as zero.
