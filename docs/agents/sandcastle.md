...
| `agent:review` | a PR | Agent reviews the diff (via the `code-review` skill) against the linked issue and `CODING_STANDARDS.md`, commits improvements, posts inline comments, marks the PR ready for review, and applies the `testflight` label for a device build (skipped for diffs that can't affect the binary). |
