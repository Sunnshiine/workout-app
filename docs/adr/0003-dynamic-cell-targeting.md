...
- **Row**: scan the day's column group for the exercise name, then resolve the target Set row relative to that anchor. In compact layouts where the Exercise header row is also Set 1, an empty header Notes cell is writable for Set 1, and Set 2 starts on the first continuation row. If the header Notes cell is non-empty and does not match the expected app-written value, it is treated as protected Coach Note or Legacy Log content and Set Logs fall back to safe continuation rows or conflict.
