...
This keeps the coach's workflow untouched (coach edits Sheet; athlete sees changes on next sync) and avoids building a dedicated backend for a single-athlete app. Local-first protects against gym connectivity loss without requiring conflict-resolution infrastructure: the app only writes new Set Log data to rows the coach never touches, so there is no write conflict between coach edits and athlete logs.
