...
`scripts/crap.sh gate` runs in the `swift-tests` CI job. CI passes `--xcodebuild`, which runs the same
package tests through xcodebuild's compilation cache instead of `swift test --enable-code-coverage`. It
